...
edge_img = 'images_and_texts/edge_flower.png'
stego_img = 'images_and_texts/stego_edge_flower.png'
...
with Image.open(edge_img) as edge:
	...
	text_bin_indx = int(text_bin_indx/8)
	if len(text_bin) > text_bin_indx+1:
		print('The image does not have the capacity to hold your message,'
			f' so the message will be cut off after {int(text_bin_indx/8)} out of {len(text)} characters.')
		print(f"Here are the last 12 characters embedded: {bin_to_text(text_bin[text_bin_indx-96:text_bin_indx])}")
	
	# print(bin_to_text(text_bin[:text_bin_indx]))
